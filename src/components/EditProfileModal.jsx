...
const EditProfileModal = ({ isVisible, onClose, id }) => {
  ...
  return (
    <div onClick={handleClose} id='wrapper' className='fixed inset-0 bg-opacity-25 backdrop-blur-sm flex justify-center items-center'>
      <div className='w-[85%] md:w-3/5 lg:w-2/5 h-[90%] bg-emptracky-darkgray rounded-xl p-4 text-emptracky-fd'>
        <div className='w-full h-fit flex mt-2'>
          <button onClick={() => onClose()} className='w-fit'>
            <Backspace size={24} />
          </button>
          <h1 className='w-full text-center text-xl md:text-3xl '>Edit your details</h1>
        </div>
        <div className='px-8 text-md md:text-sm mt-5'>
          <div className='flex flex-col gap-3'>
            ...
            <div className='flex flex-col gap-2'>
              ...
              <select id='gender' className='text-emptracky-darkgray rounded-2xl p-2'>
                <option className="md:text-base text-xs " >Male</option>
                <option className="md:text-base text-xs ">Female</option>
              </select>
            </div>
            <Button title='Save changes' onClick={handleEditProfile} styleUi="bg-emptracky-blue mt-3 "/>
          </div>
        </div>
      </div>
    </div>
