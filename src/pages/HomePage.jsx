    ...
    <DashboardElement>
      <div className="flex-col w-full h-[17%] bg-gradient-to-b from-emptracky-blue to-emptracky-f7 rounded-t-[10px] px-2 md:px-14 pt-5 pb-3 ">
        ...
        <div className="flex justify-end items-center gap-1">
          ...
          <div className='flex justify-end items-center w-[32%] md:w-[10%] lg:w-[10%]'>
            <select onChange={(e) => handleSort(e.target.value)} className="flex w-full h-[70%] rounded-md text-base md:text-lg text-[#807F7F] hover:bg-gray-200 transition-all duration-200 focus:outline-none hover:cursor-pointer">
              <option className="text-xs md:text-lg" value="name">Name</option>
              <option className="text-xs md:text-lg" value="division">Division</option>
              <option className="text-xs md:text-lg" value="contract">Contract</option>
            </select>
          </div>
        </div>
      </div>
        ...
        {data?.filter((data) => data.name.toLowerCase().includes(keyword.toLowerCase())).map((data, index) => {
            return (
              <div key={index} onClick={() => navigate(`/employee/${data.id}`)} className="flex justify-between w-full h-14 bg-emptracky-f7 border border-emptracky-darkgray rounded-[15px] mb-6 px-4 hover:border-emptracky-blue hover:text-emptracky-blue hover:cursor-pointer transition-all duration-200">
                <div className="flex justify-start items-center md:text-lg max-w-[85%] md:max-w-[50%] overflow-hidden text-base">
                  <h1 className='text-base md:text-lg'>{data.name}</h1>
                </div>
                <div className="flex h-full max-w-[5%] md:max-w-[60%] items-center">
                  <div className="h-full flex flex-col justify-center w-4 md:w-48 overflow-hidden text-[8px] md:text-base">
                    <h1 className="hidden md:block font-medium ">Division:</h1>
                    <p className="hidden md:block">{data.division}</p>
                  </div>
                  <div className="h-full w-60 flex flex-col justify-center overflow-hidden text-[8px] md:text-base">
